...
모든 모듈(Common + Identity + Core)을 포함하는 전체 애플리케이션입니다.
...
| [Core](../../modules/core/README.md)         | 비즈니스 로직 (시스템 설정)                |
...
```bash
cd apps/full
java -jar build/libs/app-full.jar
```
...
```
http://localhost:7904/monitor/health
```
...
- `logs/app-full.log`: 일반 애플리케이션 로그
- `logs/app-full-audit.log`: 감사 로그
